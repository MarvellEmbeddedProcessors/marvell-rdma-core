	...
	case IBV_QPS_RTS:
		...
		*flags = IBV_QP_STATE | IBV_QP_SQ_PSN;
		if (g_ctx.qp_type == IBV_QPT_RC) {
			...
			attr_mod->timeout = 14;
			attr_mod->retry_cnt = 7;
			attr_mod->rnr_retry = 7;
...
rdma_mq_init_unified(struct conn_ctx *conn)
{
	...
	for (i = 0; i < qp_count; i++) {
		...
		data->pending = (g_ctx.op_type == IBV_WR_SEND && data->dir != RDMA_UD_SEND) ?
					RDMA_UD_RECV :
					0;
		/* TCP posts receives early (before RTS) and sets rcnt there.
		 * The common block above sets j=0 for TCP since it skips
		 * posting.  Only update rcnt for non-TCP (RDMA CM, etc.)
		 * where j reflects the actual receives posted above. */
		if (conn->type != CONN_TYPE_TCP)
			data->rcnt = j;
		data->init = 0;
...
rdma_handle_rdma_send(struct qp_data *qdata, int tindex)
{
	...
	for (i = 0; i < ne; i++) {
		...
		if (qdata->rcnt < (int)g_ctx.rx_thold) {
			int n = (int)g_ctx.rx_depth - qdata->rcnt - 1;
...
rdma_mq_thread(void *arg)
{
	...
	while (!g_ctx.force_quit) {
		if (tq_range->count == 0 || tq_range->start_qp > tq_range->end_qp) {
			/* Empty range: no QPs assigned to this thread.
			 * Exit once init is done for client with finite packet count.
			 */
			if (!g_ctx.is_server && g_ctx.num_pkt_set && g_ctx.init_done)
				goto done;
			usleep(1000);
			continue;
		}
		...
		if (qid > tq_range->end_qp) {
			qid = tq_range->start_qp;
			/* Completed full scan with no active QPs; check exit */
